...
from sqlalchemy.orm import DeclarativeBase, joinedload, relationship, sessionmaker, mapped_column
...
class TaskModel(Base):
    ...
    task_id = mapped_column(String, primary_key=True, index=True)
    input = mapped_column(String)
...
class StepModel(Base):
    ...
    step_id = mapped_column(String, primary_key=True, index=True)
    task_id = mapped_column(String, ForeignKey("tasks.task_id"))
    name = mapped_column(String)
    input = mapped_column(String)
    status = mapped_column(String)
    output = mapped_column(String)
    is_last = mapped_column(Boolean, default=False)
    created_at = mapped_column(DateTime, default=datetime.datetime.utcnow)
    modified_at = mapped_column(
        DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow
    )
...
class ArtifactModel(Base):
    ...
    artifact_id = mapped_column(String, primary_key=True, index=True)
    task_id = mapped_column(String, ForeignKey("tasks.task_id"))
    step_id = mapped_column(String, ForeignKey("steps.step_id"))
    agent_created = mapped_column(Boolean, default=False)
...
class AgentDB:
    ...
    async def list_tasks(
        ...
        try:
            with self.Session() as session:
                tasks = (
                    session.query(TaskModel).order_by(TaskModel.created_at.desc())
