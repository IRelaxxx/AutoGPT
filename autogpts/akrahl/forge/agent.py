...
from forge.sdk.model import Artifact, Status
...
class ForgeAgent(Agent):
    ...
    async def create_task(self, task_request: TaskRequestBody) -> Task:
        ...
        await self.db.create_step(task.task_id, StepRequestBody(name=None, input=task.input, additional_input=task.additional_input), True)

        return await self.db.get_task(task.task_id)

    async def execute_step(self, task_id: str, step_id: str) -> Step:
        """
        ...
        step = await self.db.get_step(
            task_id=task_id, step_id=step_id
        )

        await self.db.update_step(task_id, step_id, status=Status.running.value)
